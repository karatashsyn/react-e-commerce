...
export class ApiProductRepository implements IRepository<Product>{
    ...
    async getAll(filter: productFilter): Promise<Product[]> {
        ...
        const filteredProducts:Product[] = returnedData.filter((p:any) => {return (( filters.filterByBrands(p,filter.brands) &&  filters.filterByPrice(p.price, filter.priceRange)) && (filters.tagsRegexMatch(p.tags, filter.searckey) || filters.filterByRegex(p.name, filter.searckey)))}) 
